"""Low-level SQL/pgvector ranked queries over persisted chunks."""
...
class ScoredDocumentChunk:
    """A persisted chunk paired with the raw database search score."""

    chunk: DocumentChunkRow
    score: float


SearchResults = list[ScoredDocumentChunk]
...
def search_vector(
    ...
    *,
    ...
    """Return nearest chunks with raw vector distances in a compatible embedding space."""
    _validate_k(k)
...
def search_text(session: Session, query: str, k: int) -> SearchResults:
    """Return full-text matches with PostgreSQL text-rank scores."""
    _validate_k(k)
    if not query.strip():
        raise ValueError('query must not be blank')
    return _execute_scored_search(session, _text_statement(query, k))
...
__all__ = ['ScoredDocumentChunk', 'SearchResults', 'search_text', 'search_vector']
